## JB adapting NB's code for effect sizes


library(metafor)

meta.food.calcification <- read_csv("calcification_by_foodsupply.csv")
head(meta.food.calcification2)



#### Calculate effect sizes
meta.food.calcification2<-escalc(measure="ROM",
																 m1i=high_food_mean, 
																 m2i=low_food_mean,
																 sd1i=high_food_SD,
																 sd2i=low_food_SD, 
...
res <- rma.mv(yi=LRR,V=LRR_var, mods=~CO2_level,random= ~1|Paper_no,struct="CS",method="REML",digits=4,data=meta.food.calcification2, slab=paste(Author, Year, sep=", "))
...
ggplot(data = meta.food.calcification2, aes(y = LRR, x = CO2_level, col=Taxon.big)) + geom_point(size = 2) +
